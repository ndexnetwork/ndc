...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return NDCGUI::tr("Received with");
    case RecvFromOther:
        return NDCGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return NDCGUI::tr("Sent to");
    case SendToSelf:
        return NDCGUI::tr("Payment to yourself");
    case Generated:
        return NDCGUI::tr("PoS Mined (Staked)");/*
    case RecvNDC:
        return NDCGUI::tr("Received ndc");
    case SendNDC:
        return NDCGUI::tr("Sent ndc");*/
    case Other:
        return NDCGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvNDC:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendNDC:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredOK, nCredNDC;
    wtx.GetCredit(nCredOK, nCredNDC, true);
    int64_t nCredit = nCredOK + nCredNDC;
    ...
    {
        if (nNet > 0 && nCredNDC > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvNDC, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendNDC, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvNDC;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv ndc
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendNDC;
                    sub.address = CBitcoinAddress(ckidD).ToString();
