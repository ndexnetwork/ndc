...
#include "ndcbridge.moc"

NDCBridge::NDCBridge(NDCGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
// This is just a hook, we won't really be setting the model...
void NDCBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void NDCBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void NDCBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void NDCBridge::populateOptions()
{
...
// Transactions
bool NDCBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void NDCBridge::clearRecipients()
{
    recipients.clear();
}

bool NDCBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 NDC, 1 NDC
        switch(rcp.txnTypeInd)
        {
            case TXT_OK_TO_OK:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::NDC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_OK_TO_ANON:
                formatted.append(tr("<b>%1</b> to NDC %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::NDC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to NDC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::NDC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_OK:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::NDC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsNDCBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your ndc balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::NDC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void NDCBridge::openAddressBook(bool sending)
{
...
void NDCBridge::openCoinControl()
{
...
void NDCBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void NDCBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap NDCBridge::listAnonOutputs()
{
...
void NDCBridge::populateTransactionTable()
{
...
void NDCBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void NDCBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString NDCBridge::transactionDetails(QString txid)
{
...
// Addresses
void NDCBridge::populateAddressTable()
{
...
void NDCBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void NDCBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString NDCBridge::newAddress(bool own)
{
...
QString NDCBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void NDCBridge::updateAddressLabel(QString address, QString label)
{
...
bool NDCBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool NDCBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void NDCBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void NDCBridge::appendMessage(int row) {
...
void NDCBridge::populateMessageTable()
{
...
void NDCBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool NDCBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool NDCBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString NDCBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool NDCBridge::setPubKey(QString address, QString pubkey)
{
...
bool NDCBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
QVariantMap NDCBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap NDCBridge::listLatestBlocks()
{
...
QVariantMap NDCBridge::findBlock(QString searchID)
{
...
QVariantMap NDCBridge::blockDetails(QString blkHash)
{
...
QVariantMap NDCBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap NDCBridge::txnDetails(QString blkHash, QString txnHash)
{
