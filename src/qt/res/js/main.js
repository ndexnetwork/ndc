...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uOK",
                this.display = "&micro;NDC";
                break;

            case 3:
                this.name    = "sOK",
                this.display = "OKtoshi";
                break;

            default:
                this.name = this.display = "NDC";
        }
...
var overviewPage = {
    init: function() {
        ...
        // Announcement feed
//        $.ajax({
//            url:"https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&q=http://updates.ndc.co/rss",
//            dataType: 'jsonp'
...
//        });
    ...
    updateBalance: function(balance, ndcBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            ndcBal   = this.ndcBal  .data("orig"),
        ...
        this.formatValue("ndcBal",   ndcBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. PzjjetSdTwrppUwwNUo1GFHYTibzJi77jM)" placeholder="Enter an NDC address (e.g. PzjjetSdTwrppUwwNUo1GFHYTibzJi77jM)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="NDC"                    ' + (unit.type == 0 ? "selected" : "") + '>NDC</option> \
                    <option value="1" title="Milli-NDC (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mOK</option> \
                    <option value="2" title="Micro-NDC (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;NDC</option> \
