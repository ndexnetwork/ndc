// Copyright (c) 2020 The nDEX Network  Developers
...
NDCGUI::NDCGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new NDCBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("NDC") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/ndc"));
    setWindowIcon(QIcon(":icons/ndc"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/ndc/index.html");
#else
    QFile html("/opt/ndc/index.html");
#endif
...
void NDCGUI::pageLoaded(bool ok)
{
...
void NDCGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void NDCGUI::urlClicked(const QUrl & link)
{
...
void NDCGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About NDC"), this);
    aboutAction->setToolTip(tr("Show information about NDC"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for NDC"));
...
void NDCGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("NDC") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/ndc_testnet"));
            setWindowIcon(QIcon(":icons/ndc_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/ndc_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("NDC client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/ndc_testnet"));
...
void NDCGUI::setWalletModel(WalletModel *walletModel)
{
...
void NDCGUI::setMessageModel(MessageModel *messageModel)
{
...
void NDCGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("NDC client"));
    trayIcon->setIcon(QIcon(":/icons/ndc"));
...
#ifndef Q_OS_MAC
void NDCGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void NDCGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to the NDC network", "", count));
}

void NDCGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void NDCGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void NDCGUI::changeEvent(QEvent *e)
{
...
void NDCGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void NDCGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::NDC, nFeeRequired));
...
void NDCGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void NDCGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void NDCGUI::gotoSignMessageTab(QString addr)
{
...
void NDCGUI::gotoVerifyMessageTab(QString addr)
{
...
void NDCGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void NDCGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NDCGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void NDCGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid NDC address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void NDCGUI::handleURI(QString strURI)
{
...
void NDCGUI::setEncryptionStatus(int status)
{
...
void NDCGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void NDCGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void NDCGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void NDCGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void NDCGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NDCGUI::updateWeight()
{
    if (!pwalletMain)
        return;
