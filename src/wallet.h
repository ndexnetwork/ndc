...
public:
    ...
    int64_t GetNDCDebit(const CTxIn& txin) const;
    int64_t GetNDCCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nOK, int64_t& nNDC) const
    {
        nOK = 0;
        nNDC = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable bool fAvailableNDCCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableNDCCreditCached;
    
    mutable int64_t nCredOKCached;
    mutable int64_t nCredNDCCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableNDCCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredOK, int64_t& nCredNDC, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredOKCached = 0;
            nCredNDCCached = 0;
            pwallet->GetCredit(*this, nCredOKCached, nCredNDCCached);
            fCreditSplitCached = true;
        };
        
        nCredOK = nCredOKCached;
        nCredNDC = nCredNDCCached;
        return true;
    }
    ...
    int64_t GetAvailableNDCCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableNDCCreditCached)
            return nAvailableNDCCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableNDCCredit() : value out of range");
            };
        };
        
        nAvailableNDCCreditCached = nCredit;
        fAvailableNDCCreditCached = true;
        return nCredit;
    };
