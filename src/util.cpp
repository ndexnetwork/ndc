...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NDC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NDC
    // Mac: ~/Library/Application Support/NDC
    // Unix: ~/.ndc
    
    
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NDC";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ndc.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
   boost::filesystem::ifstream streamConfig(GetConfigFile());
    if (!streamConfig.good())
        return; // No bitcoin.conf file is NDC
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ndcd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong NDC will not work properly.");
                    strMiscWarning = strMessage;
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("NDC"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {
