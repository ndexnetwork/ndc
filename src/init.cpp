...
{
    uiInterface.ThreadSafeMessageBox(str, _("NDC"), CClientUIInterface::BTN_OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("NDC"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    std::string strUsage = _("Options:") + "\n";
        ...
        strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ndc.conf)") + "\n";
        strUsage += "  -pid=<file>            " + _("Specify pid file (default: ndcd.pid)") + "\n";
        ...
        strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 17799 or testnet: 27799)") + "\n";
        ...
        strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 17798 or testnet: 27798)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. NDC is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  NDC is probably already running."), strDataDir.c_str()));
    
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();

    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("NDC version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
    {
        fprintf(stdout, "NDC server starting\n");
        fflush(stdout);
    };
    ...
    ********************************************************* */
    ...
    if (fs::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("NDC"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
        };
    ...
    // requested to kill ndc during the last operation. If so, exit.
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of NDC") << "\n";
        } else
        if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart NDC to complete") << "\n";
